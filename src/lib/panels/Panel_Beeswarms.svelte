<script>
  ...
  $: nodes && nodes.attr("fill", d=>tableRows.find(k=>k._index==d._index ) ? '#E485A5' : "#99999920");
...
<div class="Panel_Beeswarms[{key}] flex-1 mt-1" style="min-width:400px; max-heigth:300px;">
  <div class="flex">
    ...
    <div class="flex-1 text-right">{$M.filters[key] ? $M.filters[key].map((d,i)=>comma(i==0?Math.floor(d):Math.ceil(d))).join(' - ') : ''}</div>
  </div>
  <svg class="w-full" bind:this={el} viewBox="0 0 {beewidth+margin.left+margin.right} {beeheight+margin.top+margin.bottom}">

      <g class="x-axis" transform="translate(0, {margin.top})"></g>

      {#if xAxisLabel}
        <g transform="translate({((beewidth+margin.left)/2)}, {beeheight + margin.top + 5})">
          <text text-anchor="middle" class="text-xs fill-[#33333380]">{xAxisLabel}</text>
        </g>
      {/if}

      <g class="brush"></g>
      <g class="nodes" pointer-events="none" transform="translate(0, {margin.top})"></g>
  </svg>
</div>
